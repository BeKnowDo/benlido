...
if ( ! function_exists( 'basel_add_to_compare_btn' ) ) {
	...
	function basel_add_to_compare_btn() {
		global $product;
		echo '<div class="basel-compare-btn product-compare-button"><a class="button" href="' . esc_url( basel_get_compare_page_url() ) . '" data-added-text="' . __('Compare products', 'basel') . '" data-id="' . esc_attr( $product->get_id() ) . '">' . __('Compare', 'basel') . '</a></div>';
	}
}
...
if ( ! function_exists( 'basel_add_to_compare_single_btn' ) ) {
	...
	function basel_add_to_compare_single_btn() {
		global $product;
		echo '<a class="basel-compare-btn button" href="' . esc_url( basel_get_compare_page_url() ) . '" data-added-text="' . __('Compare products', 'basel') . '" data-id="' . esc_attr( $product->get_id() ) . '">' . __('Compare', 'basel') . '</a>';
	}
}
			...
			if ( ! empty( $products ) ) {
								...
								<?php else: ?>
									<div class="basel-compare-col compare-field">
										<?php 
											...
											echo $field_name ? esc_html( $field_name ) : esc_html( $field );
										?>
									</div>
								<?php endif; ?>
				...
				?>
					...
					<?php if ( $empty_compare_text ) : ?>
						<div class="basel-empty-compare-text"><?php echo wp_kses( $empty_compare_text, array('p' => array(), 'h1' => array(), 'h2' => array(), 'h3' => array(), 'strong' => array(), 'em' => array(), 'span' => array(), 'div' => array() , 'br' => array()) ); ?></div>
					<?php endif; ?>
		...
		switch ( $type ) {
			case 'basic':
				echo '<div class="compare-basic-content">';
					...
					echo '<h4 class="product-title"><a href="' . get_permalink( $product['id'] ) . '" target="_blank">' . $product['basic']['title']. '</a></h4>';
