...
class WC_COG_Admin_Products {
	...
	public function add_cost_field_to_simple_product() {
		...
		woocommerce_wp_text_input(
			array(
				...
				'label'     => sprintf( __( 'Cost of Good (%s)', 'woocommerce-cost-of-goods' ), get_woocommerce_currency_symbol() ),
				'data_type' => 'price',
			)
		);
	}
	...
	public function add_cost_field_to_product_variation( $loop, $variation_data, $variation ) {
		...
		<div>
			<p class="form-row form-row-first">
				<label><?php
					/* translators: Placeholder: %s - currency symbol */
					printf( esc_html__( 'Cost of Good: (%s)', 'woocommerce-cost-of-goods' ), esc_html( get_woocommerce_currency_symbol() ) ); ?></label>
	...
	public function add_quick_edit_inline_values( $column ) {
		...
		if ( 'name' === $column ) {
			...
			$meta_value = SV_WC_Product_Compatibility::get_meta( $the_product, $meta_key );
