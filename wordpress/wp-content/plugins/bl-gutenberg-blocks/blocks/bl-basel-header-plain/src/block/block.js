...
registerBlockType( 'cgb/block-bl-basel-header-plain', {
	 ...
	 */
	edit: function( props ) {
		// Creates a <p class='wp-block-cgb-block-bl-basel-header-plain'></p>.

		var super_header = props.attributes.super_header;
		var header = props.attributes.header;
		var focus = props.focus;

		function onChangeSuperHeader( updatedContent ) {
			props.setAttributes( { super_header: updatedContent } );
		}
		function onChangeHeader( updatedContent ) {
			props.setAttributes( { header: updatedContent } );
		}

		var leftLine = el('span',
							{
								className: ['left-line']
							}
						);
		var rightLine = el('span',
						{
							className: ['right-line']
						}
					);
		var titleSeparator = el('span',{
		...
		var linerContiner = el('div',
						{
							className: ['liner-continer']
						},
						leftLine,
						h4,
						rightLine
		...
		var subTitle = el(PlainText,
						{
							tagName: 'span',
							className: classNames('title-subtitle', 'font-default'),
							value: super_header,
							onChange: onChangeSuperHeader,
							focus: focus,
							onFocus: props.setFocus,
		...
		return (
			el(
				'div',
				{
					className: classNames('title-wrapper',  'basel-title-color-default', 'basel-title-style-cross', 'basel-title-size-default', 'text-center')
				},
				subTitle,
				linerContiner
			)
	...
	save: function( props ) {
		...
		return (
			...
			>
				<span className={classNames('title-subtitle', 'font-default')} >{props.attributes.super_header}</span>
				<div className={classNames('liner-continer')}> 
					<span className={classNames('left-line')}></span> 
					<h4 className={classNames('title')} >{props.attributes.header}
